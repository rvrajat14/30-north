                    ...
                    if response.result.isSuccess {
                        if let items: [Item] = response.result.value {
                            ...
                            if(items.count > 0) {
                                ...
                                _ = SweetAlert().showAlert(language.searchTitle, subTitle: language.itemNotFount, style: AlertStyle.customImag(imageFile: "Logo"))
                            }
                        }
...
extension ItemsMapViewController: MKMapViewDelegate {
    ...
    func mapView(_ mapView: MKMapView, annotationView view: MKAnnotationView, calloutAccessoryControlTapped control: UIControl) {
        ...
        itemDetailController!.selectedShopId = 1
        updateBackButton()
    }
