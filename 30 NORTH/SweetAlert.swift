...
open class SweetAlert: UIViewController {
    ...
    fileprivate func setupTitleLabel() {
		...
		titleLabel.font = UIFont(name: AppFontName.regular, size:20)
    ...
    fileprivate func setupSubtitleTextView() {
		...
		subTitleTextView.font = UIFont(name: AppFontName.regular, size:15)
        subTitleTextView.textColor = UIColor.white
        subTitleTextView.isEditable = false
        subTitleTextView.backgroundColor = UIColor.black
    }
    ...
    fileprivate func resizeAndRelayout() {
        ...
        for i in 0 ..< buttons.count {
                ...
                buttons[i].frame = CGRect(x: buttonX, y: y, width: buttonRect[i].size.width + 20.0, height: /*buttonRect[i].size.height*/ kButtonHeight)
		...
		y += kHeightMargin + buttons[0].frame.size.height + 20.0
        if y > kMaxHeight {
    ...
    open func showAlert(_ title: String, subTitle: String?, style: AlertStyle,buttonTitle: String, action: ((_ isOtherButton: Bool) -> Void)? = nil) -> SweetAlert {
        _ = self.showAlert(title, subTitle: subTitle, style: style, buttonTitle: buttonTitle,buttonColor: UIColor.colorFromRGB(0xFFFFFF))
        userAction = action
        return self
    }
    ...
    open func showAlert(_ title: String, subTitle: String?, style: AlertStyle,buttonTitle: String,buttonColor: UIColor,otherButtonTitle:
        String?, otherButtonColor: UIColor?,action: ((_ isOtherButton: Bool) -> Void)? = nil) {
            ...
            case let .customImag(imageFile):
				let newLogo = imageFile == "Logo" ? "newLogo" : imageFile
            ...
            if subTitle != nil {
				self.subTitleTextView.text = subTitle?.uppercased()
            }
            buttons = []
            if buttonTitle.isEmpty == false {
                let button: UIButton = UIButton(type: UIButton.ButtonType.custom)
				button.setTitle(buttonTitle.uppercased(), for: UIControl.State())
				button.titleLabel?.font = UIFont(name: AppFontName.bold, size: 18)
				button.backgroundColor = UIColor.white//buttonColor
            ...
            if otherButtonTitle != nil && otherButtonTitle!.isEmpty == false {
                let button: UIButton = UIButton(type: UIButton.ButtonType.custom)
				button.setTitle(otherButtonTitle?.uppercased(), for: UIControl.State())
				button.titleLabel?.font = UIFont(name: AppFontName.bold, size: 18)
                button.backgroundColor = UIColor.white//otherButtonColor
                button.setTitleColor(UIColor.black, for:.normal)
    ...
    fileprivate struct SweetAlertContext {
        static var shouldNotAnimate = true
    }
}
