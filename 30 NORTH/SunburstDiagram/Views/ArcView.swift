...
struct ArcView: View {
    ...
    var body: some View {
        ...
        return ZStack() {
            ...
            arcShape.stroke(Color.white, lineWidth: isNodeSelected() ? 7 : 0).clipShape(arcShape).animation(animation)
			...
			} else if (configuration.maximumRingsShownCount == nil || (arc.level > configuration.maximumRingsShownCount!)) && arc.width > 1 {
				ArcLabel(arc, configuration: configuration).animation(animation)
			}
        }
    }
    ...
    var body: some View {
        VStack() {
            IfLet(arc.node.image) { image in
                Image(uiImage: image).resizable().frame(width: 30, height: 30)
            }
            if !arc.isTextHidden {
                 if(UIScreen.main.bounds.height <= 667){
                    Text(arc.node.name).font(.custom(AppFontName.bold, size: 8)).foregroundColor(.white)
                 }else{
                    Text(arc.node.name).font(.custom(AppFontName.bold, size: 10)).foregroundColor(.white)
                }
            }
        }
